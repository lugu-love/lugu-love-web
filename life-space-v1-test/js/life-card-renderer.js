(function () {
  ...
  function textParts(card, options) {
    ...
    const editable = (field, placeholder) => directEdit
      ...
      : "";
    const configuredModule = window.LifeSpaceDefaults && window.LifeSpaceDefaults.channels
      ? window.LifeSpaceDefaults.channels.find((item) => item.id === (card.moduleType || card.channelId))
      : null;
    const moduleLabel = configuredModule ? configuredModule.shortName : "生活";
    const tags = [moduleLabel].concat((card.tags || []).filter(Boolean).slice(0, 2));
    const management = options && options.mode === "list"
      ? `<button class="visual-view" type="button" data-card-view="${escapeHtml(card.id)}" aria-label="查看这条${escapeHtml(moduleLabel)}记录的全部内容">查看</button>
         ${options.readonly ? "" : `<button class="visual-manage" type="button" data-card-manage="${escapeHtml(card.id)}" aria-label="管理这条${escapeHtml(moduleLabel)}记录">管理</button>`}`
